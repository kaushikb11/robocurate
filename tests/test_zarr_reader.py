"""Tests for the generic configurable Zarr reader (needs zarr; the `zarr` extra).

Builds synthetic Zarr stores with zarr in BOTH supported layouts — a root-level ``traj_*``
store with a flat ``obs`` array (read with :meth:`ZarrSchema.maniskill_like`) and a nested
``data/demo_*`` store with an ``obs`` group of named low-dim keys (read with
:meth:`ZarrSchema.robomimic_like`) — and checks the conversion to canonical trajectories:
correct feature shapes/roles/values, deterministic episode order + fingerprint, the
image-hint guard, that the source store is never mutated (invariant 1), and that the
...
from robocurate.adapters import ZarrReader, ZarrSchema
...
def _hash_store(path: Path) -> str:
    """SHA-256 over the (sorted) contents of every file in a Zarr directory store."""
    import hashlib
...
def test_maniskill_like_flat_array_layout(tmp_path: Path) -> None:
    path = tmp_path / "traj.state.zarr"
    _write_maniskill_like(path, num_traj=3, horizon=8)
    reader = ZarrReader(path, schema=ZarrSchema.maniskill_like())
    ...
    np.testing.assert_allclose(
        np.asarray(first.feature("action"), dtype=float), np.asarray(src["traj_0"]["actions"])
    )
    np.testing.assert_allclose(
        np.asarray(first.feature("observation.state"), dtype=float),
        np.asarray(src["traj_0"]["obs"])[:8],
    )
    np.testing.assert_allclose(
        np.asarray(first.feature("reward"), dtype=float),
        np.asarray(src["traj_0"]["rewards"]).reshape(-1),
    )


def test_robomimic_like_group_obs_layout(tmp_path: Path) -> None:
    path = tmp_path / "low_dim.zarr"
    _write_robomimic_like(path, num_demos=3, horizon=12)
    reader = ZarrReader(path, schema=ZarrSchema.robomimic_like())
    ...
    src = zarr.open_group(str(path), mode="r")
    obs = src["data"]["demo_0"]["obs"]
    expected = np.concatenate([np.asarray(obs[k]) for k in sorted(_GROUP_OBS_KEYS)], axis=1).astype(
        "f4"
    )
    np.testing.assert_allclose(
...
def test_deterministic_order_and_fingerprint(tmp_path: Path) -> None:
    path = tmp_path / "traj.zarr"
    _write_maniskill_like(path)
    a = ZarrReader(path, schema=ZarrSchema.maniskill_like())
    b = ZarrReader(path, schema=ZarrSchema.maniskill_like())
    assert a.fingerprint() == b.fingerprint()
    assert [t.meta.extra["source_group"] for t in a] == [t.meta.extra["source_group"] for t in b]
    # natural sort by trailing integer: traj_0 < traj_1 < traj_2.
    assert [t.meta.extra["source_group"] for t in a] == ["traj_0", "traj_1", "traj_2"]


def test_image_hint_obs_key_raises(tmp_path: Path) -> None:
    path = tmp_path / "with_image.zarr"
    root = zarr.open_group(str(path), mode="w")
    g = root.create_group("data").create_group("demo_0")
    ...
    with pytest.raises(NotImplementedError, match="image observation"):
        ZarrReader(path, schema=ZarrSchema.robomimic_like())


def test_source_store_is_not_mutated(tmp_path: Path) -> None:
    path = tmp_path / "traj.zarr"
    _write_maniskill_like(path)
    before = _hash_store(path)
    reader = ZarrReader(path, schema=ZarrSchema.maniskill_like())
    _ = list(reader)
    _ = reader.fingerprint()
    after = _hash_store(path)
